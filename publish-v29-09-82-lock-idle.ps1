...
$stage = Join-Path $root ".publish-v29-09-82"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Lock idle+live displays by default; require ?k= unless Master unlocks (v29.09.82)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.82"
