...
$stage = Join-Path $root ".publish-v29-09-94"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("master-admin.html","master-admin-app.js","entity-management.js")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Consolidate clubLocations + clubs for Master Admin URLs/display settings (v29.09.94)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.94"
