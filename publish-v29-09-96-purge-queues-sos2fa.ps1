...
$stage = Join-Path $root ".publish-v29-09-96"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Purge pending queues with Stripe refunds; Manage Entities; SOS2FA 1h (v29.09.96)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.96"
