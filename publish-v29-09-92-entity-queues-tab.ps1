...
$stage = Join-Path $root ".publish-v29-09-92"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("master-admin.html")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Move All ShoutOut Queues under Entity Management (v29.09.92)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.92"
