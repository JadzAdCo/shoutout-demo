...
$stage = Join-Path $root ".publish-v29-09-80"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display access log: real table + hostname/MAC columns; clarify IP dual-stack (v29.09.80)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.80"
