...
$stage = Join-Path $root ".publish-v29-09-71"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("suprstar-preview.html","suprstar-preview.js","payment-return-app.js","payment-return.html")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Recover supRstar preview token from sessionStorage (v29.09.71)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.71"
