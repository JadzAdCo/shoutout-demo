...
| **Next** | **FloqAi Search iteration -> production** | Preview: `floqai-search.html`. Animated FloqAi mark idles (up to Patron Status), auto pop->explode into "Tell, how can I help you?", click opens contextual search. Shipping note: keep as preview until merge decision with `?start=intent`. |
| **Next** | **SMS/WhatsApp ops messaging** | Club Admin REP alerts for pending ShoutOuts (Twilio SMS/WhatsApp), daily auth codes, inbound APPROVE/REJECT. Secrets + functions in `DEPLOYMENT-V29-09.10.md`. Paid SMS $10 checkout unchanged. |
