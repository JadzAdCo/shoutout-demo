...
| **Ask FloqR / FloqAi** | Production intent Search (`?start=intent`): FloqR logo + animated FloqAi mark that idles, speaks, and opens the wish box (“Tell, how can I help you?”). Routes to Mingl, RydR, BartR, ShoutOut, venues. Account via top-right profile card. | Classic category tile Search only; static wish box without FloqAi |
...
| **Next** | **FloqAi Search polish** | FloqAi is live on `?start=intent` (Ask FloqR). Continue polish: speech timing, mobile bounds, and feature-gate UX. `floqai-search.html` redirects to production intent. |
