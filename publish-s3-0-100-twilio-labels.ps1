...
$stage = Join-Path $root ".publish-s3-0-100-twilio-labels"
...
$files = @(
  ...
  "README.md",
  "scripts/patch-master-twilio-i18n.js"
)
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "fix: Twilio/SendGrid friendly tab labels + Ad Campaign Mgmt (s3.0.100)"
...
Write-Host "Published s3.0.100 Twilio/SendGrid labels to main"
