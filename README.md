...
- s3.0.35: Classic B&W pre-shoutout idle uses dark board + white text (lower LED current); 64âÃ¹48 main text cap 48 chars; emoji on non-B&W templates; `classic-bw-pre-shoutout.mdc` rule.
...
- s3.0.24: trim SOS2FA Privilege Admin Access help copy (Email and/or SMS; drop SMS-only path note).
...
- Use `url-media-field.js` (`FLOQRUrlMediaField.bind`) to upload to Firebase Storage and store the resulting URL internally âÃ³ÎÃ©Â¼ÎÃÂ¥ do **not** expose a visible âÃ³ÎÃ©Â¼â¼Ã´paste URLâÃ³ÎÃ©Â¼â¬Â¥ field for logos/icons unless a legacy migration path is explicitly required.
- **Display URLs** for venue LED / Xibo boards (`display.html?location=âÃ³ÎÃ©Â¼â¬Âª` and `display2.html?location=âÃ³ÎÃ©Â¼â¬Âª`) must stay **stable** (no `?v=` cache-bust query). Configure once in Xibo; do not change the URL when app versions update. Cache-bust JS/CSS only inside the HTML (`script`/`link` `?v=`). Use `FLOQRNav.stableDisplayUrl` / `stableSecondaryDisplayUrl`.
- **After every webapp modification**, first verify GitHub Actions / Functions CI logs are green for the published commit, then email mobile preview links so testers can open live URLs on iPhone: Master Admin âÃ³ÎÃÃ¡ÎÃÃ **Diagnostics** âÃ³ÎÃÃ¡ÎÃÃ **Email mobile preview links**, or deploy/call Cloud Function `emailFloqrPreviewLinks`. See `STANDARD-AFTER-DEPLOYMENT.md` steps 7âÃ³ÎÃ©Â¼ÎÃÂ£8.
- **Product spelling:** always **ShoutOut** (capital O) in user-facing copy âÃ³ÎÃ©Â¼ÎÃÂ¥ not âÃ³ÎÃ©Â¼â¼Ã´ShoutoutâÃ³ÎÃ©Â¼â¬Â¥, âÃ³ÎÃ©Â¼â¼Ã´SHOUTOUTâÃ³ÎÃ©Â¼â¬Â¥, or âÃ³ÎÃ©Â¼â¼Ã´Shout OutâÃ³ÎÃ©Â¼â¬Â¥.
...
- **BartR** shared ecommerce frontend (barter + swag). Larger Search icon tile âÃ³ÎÃÃ¡ÎÃÃ `commerce.html`. US sellers manage products in My Profile âÃ³ÎÃÃ¡ÎÃÃ BartR Store. FloqR MoR; vendor ships.
- Contextual Back (`floqr-nav.js`): satellites âÃ³ÎÃÃ¡ÎÃÃ Profile and Settings; portals âÃ³ÎÃÃ¡ÎÃÃ Search (`?start=search`).
...
1. Search BartR tile size + icon âÃ³ÎÃÃ¡ÎÃÃ BartR âÃ³ÎÃÃ¡ÎÃÃ Back to Search.
2. Portal âÃ³ÎÃÃ¡ÎÃÃ Mingl/Services/BartR (`from=portal`) âÃ³ÎÃÃ¡ÎÃÃ Back to Profile and Settings.
...
- Fixes idle ShoutOut display fallback text so a non-Zebbies location with stale/default live content shows its own venue default, such as `USE SHOUT OUT @ SHâÃ¢ÎÃÂ¥KO` or `USE SHOUT OUT @ JOSEPHINE`.
...
- Adds a known static ShâÃ¢â¬â¤ko Barcelona alias so `ShâÃ¢â¬â¤ko Barcelona Beach Club` resolves to `ShâÃ¢â¬â¤ko Barcelona` and the duplicate no longer appears as a separate active static listing.
...
Open Master Admin > Duplicate Records when two public club profiles represent the same institution, such as `ShâÃ¢â¬â¤ko Barcelona Beach Club` and `ShâÃ¢â¬â¤ko Barcelona`.
...
Guest List - ShâÃ¢â¬â¤ko Barcelona:
...
Guest List âÃ³ÎÃ©Â¼ÎÃÂ¥ Zebbies Garden DC:

```text
https://jadzadco.github.io/shoutout-demo/guest-list.html?location=zebbies-garden-washington-dc&v=28.8
```

Guest List âÃ³ÎÃ©Â¼ÎÃÂ¥ ShâÃ¢â¬â¤ko Barcelona:
...
Club Admin âÃ³ÎÃ©Â¼ÎÃÂ¥ Zebbies Garden DC:
...
6. Confirm ShâÃ¢â¬â¤ko or Christie records do not appear on Zebbies admin.
...
5. Wait 1âÃ³ÎÃ©Â¼ÎÃÂ£3 minutes.
...
# v28.8 Hard Fix âÃ³ÎÃ©Â¼ÎÃÂ¥ ShoutOut Button on Club Options
...
# v28.9 Codex Release âÃ³ÎÃ©Â¼ÎÃÂ¥ Clean Club Options + Contextual Search
...
# v28.10 Codex Release âÃ³ÎÃ©Â¼ÎÃÂ¥ Search Display Cleanup
...
# v28.11 Codex Release âÃ³ÎÃ©Â¼ÎÃÂ¥ Template Preview Cleanup
...
# v28.12 Codex Release âÃ³ÎÃ©Â¼ÎÃÂ¥ Avatar Dropdown Link Color Fix
...
- Removed the duplicated generic `âÃ³ÎÃÃ¡â¬Ã Back` button.
- Standardized workflow back buttons as page-specific `âÃ³ÎÃÃ¡â¬Ã Back to ...` controls.
- Added clearer workflow labels such as `Screen 3B âÃ³ÎÃ©Â¼ÎÃÂ¥ Club / Search` and `Screen 4 âÃ³ÎÃ©Â¼ÎÃÂ¥ Club / Select ShoutOut Template`.
