# CURRENT PACKAGE: Jadz AdCo ShoutOut v28.18-f Sign-Out + Screen 2 Cleanup Fix Package
...
- Added a centered `Or` divider before `Throw a ShoutOut` so patrons are guided to search activities or throw a ShoutOut.
...
- Bumped cache-busting query strings to `v=28.18-f`.
...
- Database rollback: no database rollback is needed for v28.18-f because this release does not require Firestore/Storage rules, index, or config changes.
...
# Jadz AdCo ShoutOut v28.18-f Navigation + Messages Fix Package
...
- Consolidated the previous v28.16 media upload/display pipeline fix and the Classic Black & White board size refinement into one `28.18-f` fix release.
...
```text
Upload v28.18-f navigation messages fix package
```
...
No Firestore or Storage rollback is needed for v28.18-f.
