...
$stage = Join-Path $root ".publish-v29-09-87"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @(
  "master-admin.html","master-display-security.js","admin.css",
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display Security Current Status under venue; Club Admin status tab (v29.09.87)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.87"
