...
$stage = Join-Path $root ".publish-v29-09-81"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "IPv6 allowlist CIDR + import IPs from log; idle note; access log table (v29.09.81)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.81"
