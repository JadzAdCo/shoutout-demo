...
$stage = Join-Path $root ".publish-v29-09-99"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("master-admin.html", "master-display-security.js", "admin.css")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Replace obfuscated URL previews with View Obfuscated Keys table (v29.09.99)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.99"
