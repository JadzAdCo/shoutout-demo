...
$stagingRoot = [IO.Path]::GetFullPath((Join-Path $packageRoot ".publish-v29-09-67-suprstar-fix"))
...
try {
  ...
  if ($LASTEXITCODE -eq 0) {
    Write-Host "supRstar flow fix is already live - no changes to commit."
  } else {
    & $gitExecutable commit -m "Fix supRstar: preview opens in new tab, payment-return has back link (v29.09.67)"
...
if (Test-Path -LiteralPath $stagingRoot) { Remove-Item -LiteralPath $stagingRoot -Recurse -Force }
Write-Host "FLOQR v29.09.67 supRstar fix published."
Write-Host "Test URL: https://jadzadco.github.io/shoutout-demo/suprstr-search.html"
