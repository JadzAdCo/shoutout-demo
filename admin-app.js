...
/* admin-app.js v18 - Admin approval queue only */
(function () {
  "use strict";
  ...
  const club = window.SHOUTOUT_CLUBS[clubId] || { name: clubId, brand: clubId };
  ...
  async function loginGoogle() { try { setText("adminStatus","Opening Google sign-in..."); await auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()); } catch(e) { console.error(e); setText("adminStatus", `${e.code || "error"}: ${e.message}`); } }
  async function loginFacebook() { try { setText("adminStatus","Opening Facebook sign-in..."); await auth.signInWithPopup(new firebase.auth.FacebookAuthProvider()); } catch(e) { console.error(e); setText("adminStatus", `${e.code || "error"}: ${e.message}`); } }
  async function loginMicrosoft() { try { const p = new firebase.auth.OAuthProvider("microsoft.com"); p.setCustomParameters({prompt:"select_account"}); setText("adminStatus","Opening Microsoft sign-in..."); await auth.signInWithPopup(p); } catch(e) { console.error(e); setText("adminStatus", `${e.code || "error"}: ${e.message}`); } }
  ...
  function renderQueue() {
    ...
    db.collection("shoutouts").where("club","==",clubId).where("status","==","pending").orderBy("submittedAt","desc")
      .onSnapshot(snapshot => {
        queueList.innerHTML = "";
      ...
      }, e => { console.error(e); queueList.innerHTML = `<p class="status">${esc(e.message)}</p>`; });
  }

  async function approve(id, item) {
    await db.collection("liveContent").doc(clubId).set({
      club: clubId,
      clubName: item.clubName || club.name,
  ...
  document.addEventListener("DOMContentLoaded", () => {
    setText("clubName", club.name);
    setText("adminStatus", "Admin app loaded. Sign in to continue.");
    byId("displayLink").href = `./display.html?club=${clubId}`;
    byId("liveFrame").src = `./display.html?club=${clubId}`;
