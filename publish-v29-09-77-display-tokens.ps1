...
$stage = Join-Path $root ".publish-v29-09-77"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display board tokens (?k=) + Master Admin rotate/copy Xibo URLs (v29.09.77)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published frontend v29.09.77"
