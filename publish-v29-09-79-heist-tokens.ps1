...
$stage = Join-Path $root ".publish-v29-09-79"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display Security: re-issue BOTH board tokens for Heist/venue testing (v29.09.79)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.79"
