...
$stage = Join-Path $root ".publish-s3-0-71-google-login"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("index.html", "patron-app.js")
...
$msg = "s3.0.71: harden Google login redirect fallback + Guest Sign in menu"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    & $git log -1 --oneline
  } else { Write-Host "No Pages file changes" }
} finally { Pop-Location }

Write-Host "Published s3.0.71 Google login hardening"
