...
async function loginGoogle(){if(window.FLOQRSessionShell?.popupBlocked?.("#guestStatus"))return;try{setText("guestStatus","Opening Google sign-in...");await auth.signInWithPopup(new firebase.auth.GoogleAuthProvider());}catch(e){setText("guestStatus",`${e.code||"error"}: ${e.message}`);}}
