...
$stage = Join-Path $root ".publish-v29-09-75"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @(
  "suprstar-preview.html","suprstar-preview.js","suprstr-search.css"
)
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "supRstar preview: single stage button (camera → pay → wait → live) (v29.09.75)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.75"
