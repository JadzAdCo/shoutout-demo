...
$stage = Join-Path $root ".publish-v29-09-74"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "display2 idle: supRstar logo + Awaiting live Feed CTA (v29.09.74)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.74"
Write-Host "Xibo (stable): https://jadzadco.github.io/shoutout-demo/display2.html?location=heist-washington-dc"
