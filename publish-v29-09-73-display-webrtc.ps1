...
$stage = Join-Path $root ".publish-v29-09-73"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @(
  "suprstr-webrtc.js","display-suprstr.js","display2.html","display.html",
  "display.css","suprstar-preview.html"
)
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Fix supRstar display WebRTC attach + renegotiation for Xobo board (v29.09.73)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.73"
