...
$stage = Join-Path $root ".publish-v29-09-78"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @(
  "display-app.js","display2.html","display.html",
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display tokens: onboarding one-time reveal + obfuscated Security portal + rotate directives (v29.09.78)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published frontend v29.09.78"
