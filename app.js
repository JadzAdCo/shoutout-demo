/*
  Jadz AdCo ShoutOut - app.js v16 compat
  This version does not use ES module imports.
  It expects:
    1. firebase-app-compat.js
    2. firebase-auth-compat.js
    3. firebase-firestore-compat.js
    4. firebase-config.js with window.firebaseConfig
*/
...
(function () {
  ...
  function setStatus(text) { setText("authStatus", text); }
  ...
  if (!config) {
    setStatus("firebase-config.js loaded, but window.firebaseConfig was not found.");
    console.error("Missing window.firebaseConfig. Update firebase-config.js for v16 compat.");
    return;
  }
  ...
  async function loadTemplates() {
    ...
    try {
      ...
      snap.forEach(item => templates[item.id] = { id: item.id, ...item.data() });
    } catch (error) {
  ...
  async function loadClubs() {
    ...
    try {
      ...
      snap.forEach(item => clubs[item.id] = { id: item.id, ...item.data() });
    } catch (error) {
      console.warn("Using fallback clubs:", error.message);
    }
    if (Object.keys(clubs).length === 0) clubs = { ...SEED_CLUBS };
  }
  ...
  async function loadClubById(id) {
    ...
    try {
      ...
      console.warn("Could not load club:", error.message);
    }
  ...
  function renderTemplates() {
    ...
    (getClub().templates || ["neon"]).forEach(templateId => {
      ...
      item.innerHTML = `<div class="name">${escapeHTML(template.name)}</div><div class="tag">${escapeHTML(template.scope || "Shared")} template</div>`;
  ...
  function updateTemplateSummary() {
    ...
    if (box) box.innerHTML = `<h3>${escapeHTML(template.name)}</h3><p>${escapeHTML(template.scope || "Shared")} template selected.</p>`;
  }
  ...
  console.log("Jadz ShoutOut app.js v16 compat loaded");
})();
