...
$stage = Join-Path $root ".publish-v29-09-85"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("master-admin.html","master-display-security.js")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Display security: IP and token locks independent; fix forced token checkbox (v29.09.85)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.85"
