...
$files = @("master-admin.html", "master-diagnostics-panels.js", "admin.css")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Show Display/Xibo load errors in a visible Date/Time/IP/Details table (v29.09.98)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.98"
