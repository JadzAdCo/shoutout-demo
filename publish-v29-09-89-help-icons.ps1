...
$stage = Join-Path $root ".publish-v29-09-89"
$git = "C:\Program Files\Git\cmd\git.exe"
$files = @("master-admin.html","admin.css","styles.css")
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "Help icons sit beside labels site-wide (PC/tablet/phone) (v29.09.89)"
    & $git -c credential.helper= -c credential.helper=manager push origin main
  }
} finally { Pop-Location; if (Test-Path $stage) { Remove-Item $stage -Recurse -Force } }
Write-Host "Published v29.09.89"
