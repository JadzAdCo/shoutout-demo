...
test("chrome packs match en key count for every supported language", () => {
  ...
  assert.equal(chrome.enKeys, 341);
...
test("floqr-i18n and help VERSION are s3.0.85", () => {
  const fs = require("fs");
  ...
  assert.match(i18n, /const VERSION\s*=\s*"s3\.0\.85"/);
  assert.match(help, /const VERSION\s*=\s*"s3\.0\.85"/);
});
