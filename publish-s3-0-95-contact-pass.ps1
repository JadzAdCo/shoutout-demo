...
$stage = Join-Path $root ".publish-s3-0-95-contact-pass"
...
$files = @(
  ...
  "functions/scripts/recrawl-incomplete-discovery-contacts.js",
  "functions/scripts/benchmark-contact-enrich.js",
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "feat: discovery contact pass — brand domains + WhatsApp phone (s3.0.95)"
...
Write-Host "Published s3.0.95 contact completeness pass to main"
