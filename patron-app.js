...
(function () {
  ...
  function updateGlobalBackButton() {
    const btn = byId("globalBackBtn");
    if (!btn) return;
    const hide = currentPageId === "landingPage" || currentPageId === "adSplashPage";
    btn.classList.toggle("hidden", hide);
  }
  function showPage(id) {
    if (!id || !byId(id)) return;
    if (!suppressPageHistory && currentPageId && currentPageId !== id) pageHistory.push(currentPageId);
    currentPageId = id;
  ...
  function goBack() {
    ...
    if (previous && byId(previous)) showPage(previous);
    else if (currentUser && currentPageId !== "categoryPage") showPage("categoryPage");
  ...
  function ensureProfileMenuEnhancements(user) {
    ...
    if (!menu.querySelector("[data-patron-menu='portal']")) {
      ...
      portalLink.href = "./patron-portal.html?v=28.14";
      ...
      messages.href = "./patron-portal.html?tab=messages&v=28.14";
      ...
      chats.href = "./patron-portal.html?tab=chats&v=28.14";
...
(function(){
  ...
  async function enhanceMenu(user){
    ...
    menu.innerHTML = `
      ...
      <a class="profile-menu-link" href="./patron-portal.html?v=28.14">My Profile</a>
      <div class="profile-menu-line">Member Level: Patron</div>
      <a class="profile-menu-link" href="./patron-portal.html?tab=messages&v=28.14">Messages (${c.um}/${c.tm})</a>
      <a class="profile-menu-link" href="./patron-portal.html?tab=chats&v=28.14">Chats (${c.uc}/${c.tc})</a>
...
window.createShoutOutSubmissionNotification=async function(s){await note({type:"shoutoutSubmitted",title:"ShoutOut Submitted",body:`Your ShoutOut was submitted for ${s.locationName||s.clubName||s.clubLocationId||"the selected venue"}.`,referenceNumber:s.referenceNumber||"",clubLocationId:s.clubLocationId||s.location||currentLoc(),status:s.status||"pending",link:"./patron-portal.html?tab=shoutouts&v=28.14"});};
