...
(function () {
  ...
  async function recordMinglRequest({connectionId, targetUid, targetProfile, nextStatus, sharedLabels}) {
    ...
    const base = {
      ...
      status:nextStatus,
      link:"./patron-portal.html?tab=inbox&v=29.00",
      minglLink:"./mingl-chat.html?v=29.00",
      read:false,
      createdAt:now
    };
    const writes = [
  ...
  function portalChatUrl(roomId = "") {
    const params = new URLSearchParams({v:"29.00"});
  ...
  async function submitShoutout() {
    ...
    try {
      ...
      payload.modifyLink = `./patron-portal.html?tab=shoutouts&ref=${encodeURIComponent(payload.referenceNumber)}&id=${encodeURIComponent(shoutoutRef.id)}&v=29.00`;
  ...
  function ensureProfileMenuEnhancements(user) {
    ...
    if (!menu.querySelector("[data-patron-menu='portal']")) {
      ...
      portalLink.href = "./patron-portal.html?v=29.00";
      ...
      messages.href = "./patron-portal.html?tab=inbox&v=29.00";
      ...
      chats.href = "./mingl-chat.html?v=29.00";
...
(function(){
  ...
  async function enhanceMenu(user){
    ...
    menu.innerHTML = `
      ...
      <a class="profile-menu-link" href="./patron-portal.html?v=29.00">My Profile and Settings</a>
      <div class="profile-menu-line">Member Level: Patron</div>
      <a class="profile-menu-link" href="./patron-portal.html?tab=inbox&v=29.00">FLOQR Inbox (${c.um}/${c.tm})</a>
      <a class="profile-menu-link" href="./mingl-chat.html?v=29.00">Mingl (${c.uc}/${c.tc})</a>
...
window.createShoutOutSubmissionNotification=async function(s){const link=s.modifyLink||`./patron-portal.html?tab=shoutouts&ref=${encodeURIComponent(s.referenceNumber||"")}&v=29.00`;await note({type:"shoutoutSubmitted",title:"ShoutOut Submitted",body:`Your ShoutOut was submitted for ${s.locationName||s.clubName||s.clubLocationId||"the selected venue"}.\n\nModify ShoutOut: ${link}`,referenceNumber:s.referenceNumber||"",shoutoutId:s.shoutoutId||"",clubLocationId:s.clubLocationId||s.location||currentLoc(),status:s.status||"pending",link});};
