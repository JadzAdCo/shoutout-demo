...
(function () {
  ...
  function shouldUseSameTabRedirect() {
    const ua = navigator.userAgent || "";
    return document.body.classList.contains("device-mobile") || /iPhone|iPad|iPod|Android|CriOS|FxiOS/i.test(ua);
  }
  async function signInProvider(provider, label) {
    try {
      ...
      if (code === "auth/popup-blocked" || code === "auth/popup-closed-by-user" || code === "auth/cancelled-popup-request") {
        setStatus(`${label} popup was blocked or opened in another tab. Redirecting in this tab...`);
        await auth.signInWithRedirect(provider);
        return;
      }
  ...
  async function submitShoutout() {
    ...
    try {
      ...
      payload.modifyLink = `./patron-portal.html?tab=shoutouts&ref=${encodeURIComponent(payload.referenceNumber)}&id=${encodeURIComponent(shoutoutRef.id)}&v=28.36-nf`;
  ...
  function ensureProfileMenuEnhancements(user) {
    ...
    if (!menu.querySelector("[data-patron-menu='portal']")) {
      ...
      portalLink.href = "./patron-portal.html?v=28.36-nf";
      ...
      messages.href = "./patron-portal.html?tab=messages&v=28.36-nf";
      ...
      chats.href = "./patron-portal.html?tab=chats&v=28.36-nf";
...
(function(){
  ...
  async function enhanceMenu(user){
    ...
    menu.innerHTML = `
      ...
      <a class="profile-menu-link" href="./patron-portal.html?v=28.36-nf">My Profile</a>
      <div class="profile-menu-line">Member Level: Patron</div>
      <a class="profile-menu-link" href="./patron-portal.html?tab=messages&v=28.36-nf">Messages (${c.um}/${c.tm})</a>
      <a class="profile-menu-link" href="./patron-portal.html?tab=chats&v=28.36-nf">Mingl (${c.uc}/${c.tc})</a>
...
window.createShoutOutSubmissionNotification=async function(s){const link=s.modifyLink||`./patron-portal.html?tab=shoutouts&ref=${encodeURIComponent(s.referenceNumber||"")}&v=28.36-nf`;await note({type:"shoutoutSubmitted",title:"ShoutOut Submitted",body:`Your ShoutOut was submitted for ${s.locationName||s.clubName||s.clubLocationId||"the selected venue"}.\n\nModify ShoutOut: ${link}`,referenceNumber:s.referenceNumber||"",shoutoutId:s.shoutoutId||"",clubLocationId:s.clubLocationId||s.location||currentLoc(),status:s.status||"pending",link});};
