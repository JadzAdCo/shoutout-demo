...
$stage = Join-Path $root ".publish-s3-0-25-entity-mgmt-deeplink"
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    & $git commit -m "s3.0.25: Entity Management defaults to Manage Entities; deep links resume after auth+SOS2FA"
    & $git -c credential.helper= -c credential.helper=manager push origin main
    if ($LASTEXITCODE -ne 0) { throw "Pages push failed" }
    & $git log -1 --oneline
  }
} finally { Pop-Location }
Write-Host "Published s3.0.25 Pages"
